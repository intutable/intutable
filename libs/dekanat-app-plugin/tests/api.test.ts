...
import { Core, CoreRequest } from "core"
import { openConnection, closeConnection, insert } from "database/dist/requests"
...
describe("create table", () => {
    ...
    test("table with appropriate columns exists", async () => {
        ...
        const rawData = await coreRequest<lvt.ViewInfo>(lvr.getViewInfo(connId, TABLE.id))
    ...
    test("delete table", async () => {
        ...
        const tables: PmTable[] = await core.events.request(
            pm.getTablesFromProject(connId, PROJECT_ID)
        )
        expect(tables).not.toEqual(
            expect.arrayContaining([expect.objectContaining({ name: otherTableName })])
        )
    })
})
...
describe("create view", () => {
    ...
    test("create/delete view", async () => {
        ...
        expect(viewData.columns).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ [COLUMN_INDEX_KEY]: 2, name: userPrimaryColumnName() }),
            ])
        )
...
describe("rename columns", () => {
    const TABLE_SPEC = {
        ...
        columns: {
            ...
            salary: {
                specifier: { name: "Salary", cellType: "number" },
                id: -1,
            },
        },
    }
    ...
    test("valid rename - column also renamed in views", async () => {
        ...
        testTableData = await coreRequest<TableData>(req.getTableData(connId, testTable.id))
        expect(testTableData.columns).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
...
describe("row handling", () => {
    ...
    test("create a new empty row at a desired index; batch update", async () => {
        const { _id: _id1 } = await coreRequest<{ _id: number }>(
            req.createRow(connId, view.id, {
                values: {
        ...
        const { _id: _id2 } = await coreRequest<{ _id: number }>(
            req.createRow(connId, view.id, {
                atIndex: 0,
                values: {
                    [nameColumn.id]: "First",
    ...
    test("row deletion", async () => {
        ...
        const { _id: _id3 } = await coreRequest<{ _id: number }>(
            req.createRow(connId, view.id, {
                atIndex: 2,
                values: {
...
describe("links between tables", () => {
    ...
    const homeRow1 = { _id: -1, name: "Doe", salary: 60000 }
    const homeRow2 = { _id: -1, name: "Stag", salary: 60001 }

    const foreignTableName = "supervisors"
    ...
    const foreignRow1 = { _id: -1, name: "Bill Lumbergh", authorityLevel: 100000 }
    ...
    beforeAll(async () => {
        ...
        foreignTableData = await coreRequest<TableData>(req.getTableData(connId, foreignTable.id))
    ...
    test("link and lookup columns created", async () => {
        ...
        expect(lookupColumn).toEqual(
            expect.objectContaining({
                id: expect.any(Number),
                kind: "lookup",
            })
        )
        expect(backwardLinkColumn).toEqual(
            expect.objectContaining({
                id: expect.any(Number),
                name: expect.stringContaining(userPrimaryColumnName()),
                kind: "backwardLink",
        ...
        expect(backwardLookupColumn).toEqual(
            expect.objectContaining({
                id: expect.any(Number),
                kind: "backwardLookup",
            })
    ...
    test("link rows to each other", async () => {
        ...
        homeTableData = await coreRequest<TableData>(req.getTableData(connId, homeTable.id))
        expect(homeTableData.rows).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    _id: homeRow1._id,
        ...
        expect(foreignTableData.rows).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    _id: foreignRow1._id,
    ...
    test("changing a column's type also changes types of links and lookups", async () => {
        ...
        try {
            ...
            expect(changedHomeColumns[1]).toEqual({
                ...backwardLookupColumn,
                cellTypeParameter: "string",
            })
