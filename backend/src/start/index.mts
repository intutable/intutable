...
function main(){
  ...
  // I'd just use the NPM scripts, but they appear to disown the node processes
  // they launch and they then can't be killed.
  const coreProcess     = spawn("node", [ CORE_SCRIPT ])
  const frontendProcess = spawn("node", [ FRONTEND_SCRIPT, args.frontendOption])
  ...
  process.on("SIGINT", () => shutdown(components, "received interrupt signal"))
