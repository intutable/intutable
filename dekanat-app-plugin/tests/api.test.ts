...
beforeAll(async () => {
    ...
    await core.events.request(openConnection(sessionID, DB_USERNAME, DB_PASSWORD))
    ...
    ADMIN_ID = admin._id
    const projDesc = await core.events.request(pm.createProject(sessionID, ADMIN_ID, "project"))
...
describe("create table", () => {
    ...
    beforeAll(async () => {
        TABLE = await core.events.request(req.createTable(sessionID, ADMIN_ID, PROJECT_ID, TABLE_NAME))
        DATA = await core.events.request(req.getTableData(sessionID, TABLE.id))
    })

    afterAll(async () => {
        await core.events.request(req.deleteTable(sessionID, TABLE.id))
    })
    ...
    test("table with appropriate columns exists", async () => {
        ...
        const rawData = (await core.events.request(lvr.getViewInfo(sessionID, TABLE.id))) as lvt.ViewData
    ...
    test("table has default view", async () => {
        const views: lvt.ViewDescriptor[] = await core.events.request(req.listViews(sessionID, TABLE.id))
        expect(views.length).toBe(1)
        const view: SerializedViewData = await core.events.request(req.getViewData(sessionID, views[0].id))
    ...
    test("delete table", async () => {
        ...
        await core.events.request(req.deleteTable(sessionID, otherTable.id))
        let tables: PmTable[] = await core.events.request(pm.getTablesFromProject(sessionID, PROJECT_ID))
...
describe("create view", () => {
    ...
    async function createView() {
        VIEW = await core.events.request(req.createView(sessionID, ADMIN_ID, TABLE.id, VIEW_NAME))
    }
    async function deleteView() {
        await core.events.request(req.deleteView(sessionID, VIEW.id))
    }
    beforeAll(async () => {
        TABLE = await core.events.request(req.createTable(sessionID, ADMIN_ID, PROJECT_ID, TABLE_NAME))
    })
    ...
    test("create/delete view", async () => {
        await createView()
        let views = (await core.events.request(req.listViews(sessionID, TABLE.id))) as ViewDescriptor[]
        ...
        const viewData = (await core.events.request(req.getViewData(sessionID, VIEW.id))) as SerializedViewData
        expect(viewData.columns).toEqual(
            expect.arrayContaining([expect.objectContaining({ [COLUMN_INDEX_KEY]: 2, name: "Name" })])
        )

        await deleteView()
        views = (await core.events.request(req.listViews(sessionID, TABLE.id))) as ViewDescriptor[]
    ...
    test("cannot rename or delete default view", async () => {
        ...
        const deletePromise = core.events.request(req.deleteView(sessionID, defaultView.id))
        expect(deletePromise).rejects.toEqual(
            expect.objectContaining({
                code: ErrorCode.changeDefaultView,
            })
        )
        const renamePromise = core.events.request(req.renameView(sessionID, defaultView.id, NEW_NAME))
...
describe("create different kinds of columns", () => {
    ...
    beforeAll(async () => {
        TEST_TABLE = await core.events.request(req.createTable(sessionID, ADMIN_ID, PROJECT_ID, TABLE_SPEC.name))
    })

    test("create standard column", async () => {
        const column = TABLE_SPEC.columns[0]
        const newColumn = (await core.events.request(
            req.createStandardColumn(sessionID, TEST_TABLE.id, column)
        )) as SerializedColumn
