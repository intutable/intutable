...
async function getProjects(request: CoreRequest): Promise<ProjectDescriptor[]> {
    const allProjects = (await core.events.request(pm.getProjects(request.connectionId, request.unusedRoleId))) as ProjectDescriptor[]

    const roleId = await core.events.request(getRoles(request.connectionId, '', request.username))
    // TODO check every project specificly in loop?
    const permissions = await core.events.request(can(request.connectionId, roleId[0], 'read', 'project', '', ''))
