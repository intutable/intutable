 ...
 * Once the last update's
 * cooldown timeout expires, fire `callback` and `resolve` or `reject`
 * the `promise` with whatever it produced.
 * @param callback The update function to call when the timeout expires
 * @param promise the promise returned to the user who called `update`;
 * we keep it around so we can return it again in case `update` is
 * called another time.
 * @param resolve (to be called by the timeout function) resolve `promise`
 * with the result of `callback`.
 * @param reject reject `promise` in case `callback` failed, or if the timeout
 * is cancelled.
 */
...
/**
 * A timer for limiting how many times a callback is run.
 * In short, one can run an "update" callback (opaque to the hook)
 * which sets a timeout so that no more updates can happen before it expires.
 * One more call of `callback` may be queued up, additional
 * calls to `update` after this simply
 ...
 * When the timeout expires in _Waiting_ mode, revert to _Idle_ mode. When
 * the timeout expires in _Pending_ mode, switch to _Waiting_, set a
 * new timeout.
 * Notes:
 ...
 *       the data the callback operates on are correct and up-to-date is
 *       the user's responsibility.
 *    2. `update` may be called with an alternative callback ad hoc.
...
export const useUpdateTimer= <Result>(
     ...
     * if a second update was already pending.
     */
    const update = (
        ...
        if (currentTimeout.current === null && pendingUpdate.current === null){
            ...
            // we have to assign them to keep tsc happy
     ...
     * Puts the timer in _Waiting_ state, so the timeout will continue
     * until it expires as normal.
