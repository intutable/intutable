 ...
 */
export const deserializeView: Middleware = (useSWRNext: SWRHook) => (key, fetcher, config) => {
    const swr = useSWRNext(key, fetcher, config)

    if (key == null || Object.prototype.hasOwnProperty.call(key, "url") === false) return swr
    ...
    // deserialize
    const deserializedViewData: ViewData.Deserialized = SerDes.deserializeView(viewData)
